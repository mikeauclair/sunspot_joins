...
require 'ostruct'
require 'sunspot'
require 'sunspot_joins'

require File.join(Gem::Specification.find_by_name("sunspot").gem_dir, "spec", 'mocks', 'mock_record.rb')
Dir.glob(File.join(File.dirname(__FILE__), 'mocks', '**', '*.rb')).each do |file|
  require file unless File.basename(file) == 'mock_record.rb'
end

Dir.glob(File.join(Gem::Specification.find_by_name("sunspot").gem_dir, "spec", "helpers", "*.rb")).each do |file|
  puts file
  require file
end
...
require File.join(Gem::Specification.find_by_name("sunspot").gem_dir, "spec", 'ext')
